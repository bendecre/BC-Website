			...
			if(issetor($queryid)){
				echo ' <script>unique('.$id.');</script>
					<tr>
						...
						<td><textarea cols="13" rows="1" style="position: absolute; position: relative">' . $row['Synonym'] . '</textarea></td>
						<td><textarea id="comment'.$id.'" cols="12" rows="1" style="position: absolute; position: relative">'. issetor($row2['comments']) .'</textarea><br><BUTTON style="width:105px;" onClick="commentSubmit(\'comment'.$id.'\',\''.$queryid.'\');">Submit</BUTTON></td>
						...
						<td id="go'.$id.'"><textarea cols="13" rows="1" style="position: absolute; position: relative">' . $row['GO_terms'] . '</textarea></td>
						<td>' . issetor($row2['Length']) . '<BUTTON style="float: right;" onClick="copyText(\'sequence'.$id.'\');">Copy-></BUTTON></td>
						<td><textarea id="sequence'.$id.'"cols="12" rows="1">' . issetor($row2['Sequence']) . '</textarea></td>
					</tr>
				';
				$id = $id + 1;
			}
			...
			if(issetor($seqName)) {
				echo ' <script>unique('.$id.');</script>
					<tr>
						...
						<td><textarea cols="13" rows="1" style="position: absolute; position: relative">' . $row2['Synonym'] . '</textarea></td>
						<td><textarea id="comment'.$id.'" cols="12" rows="1" style="position: absolute; position: relative">'. issetor($row['comments']) .'</textarea><br><BUTTON style="width:120px;" onClick="commentSubmit(\'comment'.$id.'\',\''.$seqName.'\');">Submit</BUTTON></td>
						...
						<td><textarea cols="13" rows="1" style="position: absolute; position: relative">' . $row2['GO_terms'] . '</textarea></td>
						<td>' . issetor($row['Length']) . '<BUTTON style="float: right;" onClick="copyText(\'sequence'.$id.'\');">Copy-></BUTTON></td>
						<td><textarea id="sequence'.$id.'"cols="12" rows="1">' . issetor($row['Sequence']) . '</textarea></td>
					</tr>
				';
				$id = $id + 1;
			}	
